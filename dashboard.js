...
function render(rows){ table.innerHTML=`<tr><th>Area</th><th>Parameter</th><th>Target</th><th>Actual</th><th>Judgt</th><th>Trend</th><th>Remarks</th><th>Updated By</th></tr>`+rows.map(r=>`<tr><td>${r.area}</td><td>${r.parameter}</td><td>${r.monthTarget||''}</td><td>${r.actual??''}</td><td class='big'>${r.judgt??''}</td><td class='big'>${r.trend??''}</td><td>${r.remarks??''}</td><td>${r.updatedBy??''}</td></tr>`).join(''); }
async function load(){ const m=dashMonth.value; const saved={}; const qs=await getDocs(query(collection(db,'kpis'), where('month','==',m))); qs.forEach(s=>{saved[s.data().area+'_'+s.data().parameter]=s.data();}); const rows=KPI_MASTER.map(k=>({...k, ...(saved[k.area+'_'+k.parameter]||{})})); render(rows); msg.textContent=`Showing ${m}`; }
dashMonth.addEventListener('change', load); document.getElementById('downloadBtn').addEventListener('click', async()=>{ await load(); let csv='Area,Parameter,Month Target,Actual,Judgt,Trend,Remarks
